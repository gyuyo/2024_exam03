...
<section class="mt-8 flex-1">
	<div class="container mx-auto">
		...
		<div class="btns mx-auto w-9/12 my-3 text-sm flex justify-between">
			<button onclick="history.back();">뒤로가기</button>
			<c:if test="${rq.getLoginedMemberId() == article.getMemberId() }">
				<div class="flex">
					<a href="modify?id=${article.getId() }" class="p-5">수정</a>
					<a onclick="if(confirm('정말 삭제하시겠습니까?') == false) return false; " href="doDelete?id=${article.getId() }&boardId=${article.getBoardId() }" class="p-5  ml-2">삭제</a>
				</div>
			</c:if>
		</div>
	</div>
